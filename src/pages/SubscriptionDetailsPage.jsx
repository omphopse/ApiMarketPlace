...
const SubscriptionDetailsPage = () => {
  ...
  const pct = Math.min(100, Math.round((subscription.requestsUsed / subscription.requestLimit) * 100));
        ...
        <Grid item xs={12} md={8}>
          <AppCard title="Plan details" subtitle="Current subscription state">
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12} md={4}><Box sx={{ p: 2, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}><Typography variant="caption" color="text.secondary">Started</Typography><Typography fontWeight={700}>{subscription.startedAt}</Typography></Box></Grid>
          ...
          <AppCard title="Usage" subtitle="Current consumption" sx={{ mt: 3 }}>
            <Stack spacing={2}>
              <Box><Typography variant="caption" color="text.secondary">Requests Used</Typography><Typography variant="h4" fontWeight={700}>{subscription.requestsUsed.toLocaleString('en-IN')}</Typography></Box>
              <LinearProgress variant="determinate" value={pct} sx={{ height: 10, borderRadius: 999 }} />
              <Typography color="text.secondary">{subscription.requestLimit.toLocaleString('en-IN')} request limit • {pct}% used</Typography>
            </Stack>
          </AppCard>
        </Grid>
