...
public class DefaultAdminInitializer implements ApplicationRunner {
    ...
    private void createCollectionsIfMissing() {
        // Create subscription_plans collection if it doesn't exist
        if (!mongoTemplate.collectionExists(SubscriptionPlan.class)) {
            mongoTemplate.createCollection(SubscriptionPlan.class);
            log.info("Created 'subscription_plans' collection");
        }

        // Create api_documentation collection if it doesn't exist
        if (!mongoTemplate.collectionExists(ApiDocumentation.class)) {
            mongoTemplate.createCollection(ApiDocumentation.class);
            log.info("Created 'api_documentation' collection");
        }
    }
}
