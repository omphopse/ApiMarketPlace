  ...
  <Card sx={{ p: 3, bgcolor: 'secondary.main', height: '100%' }}>
    ...
    <Stack spacing={2} sx={{ mt: 2 }}>
      ...
      <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
        {(api?.category || api?.categoryName) && <Chip label={api.category || api.categoryName} size="small" color="primary" />}
      ...
      <Box sx={{ p: 2, borderRadius: 3, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }}>
        ...
        <Typography variant="body2" color="text.secondary">Base URL: {api?.baseUrl || 'Unavailable'}</Typography>
        <Typography variant="body2" color="text.secondary">Authentication: {api?.authenticationType || api?.authType || 'API_KEY'}</Typography>
