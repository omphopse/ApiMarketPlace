...
const ApiKeysPage = () => {
  ...
  const confirmAction = async () => {
    ...
    try {
      if (mode === 'regenerate') {
        const result = await consumerService.regenerateApiKey(selectedKey.subscriptionId || selectedKey.id);
        setNewKey(result.apiKey || result.key || '');
        setShowRawKey(true);
  ...
  return (
    <DashboardLayout role="CONSUMER" title="API keys" subtitle="Manage developer credentials securely.">
      ...
      <AppCard title="Security notice" subtitle="Never expose API keys in reusable frontend code or public repositories.">
              <Typography color="text.secondary">The full API key is shown only immediately after generation or regeneration. Existing keys remain masked in the list.</Typography>
      </AppCard>
      <AppCard title="Active keys" subtitle="Current credentials for your subscriptions" sx={{ mt: 3 }}>
        {loading ? <LoadingState title="Loading API keys" description="Fetching your developer credentials." /> : error ? <ErrorState message={error} retryLabel="Try again" onRetry={loadApiKeys} /> : apiKeys.length === 0 ? <EmptyState title="No API keys yet" description="Subscribe to an API to generate your first key." actionLabel="Explore APIs" actionTo="/marketplace" /> : <Stack spacing={2}>{apiKeys.map((key) => <Box key={key.id} sx={{ p: 2.5, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2}><Box><Typography fontWeight={700}>{key.apiName || key.label || 'API key'}</Typography><Typography color="text.secondary">{key.maskedKey || key.keyPrefix || 'No preview available'}</Typography><Typography variant="body2" color="text.secondary">Created {key.createdAt ? new Date(key.createdAt).toLocaleString() : 'Not available'} • Last used {key.lastUsedAt ? new Date(key.lastUsedAt).toLocaleString() : 'Never'}</Typography></Box><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><Chip label={key.status} color={key.status === 'ACTIVE' ? 'success' : 'default'} /><Button onClick={() => handleRegenerate(key)} variant="outlined">Regenerate</Button><Button onClick={() => handleRevoke(key)} color="error">Revoke</Button></Stack></Stack></Box>)}</Stack>}
      </AppCard>
