...
const ProviderApiDetailPage = () => {
  ...
  const summaryCards = useMemo(() => [
    { label: 'Subscribers', value: api?.subscribers != null ? formatNumber(api.subscribers) : 'Not available' },
    { label: 'Requests', value: api?.requests != null ? formatNumber(api.requests) : 'Not available' },
    { label: 'Revenue', value: api?.revenue != null ? formatCurrency(api.revenue) : 'Not available' },
  ...
  return (
    <DashboardLayout role="PROVIDER" title="API details" subtitle="Review your listing and manage approval state.">
      <PageHeader title={api.name} subtitle={api.description || 'No description provided.'} action={<Stack direction="row" spacing={1}><Button component={Link} to={`/provider/apis/${api.id}/edit`} variant="outlined">Edit</Button><Button onClick={handleSubmit} variant="contained">Submit</Button></Stack>} />
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <AppCard title="Overview" subtitle="Current marketplace presentation">
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }}>
              <Box>
                ...
                <Typography color="text.secondary">{api.categoryName || 'Category unavailable'} • {api.version}</Typography>
              </Box>
              <Chip label={statusConfig[api.status]?.label || api.status} color={statusConfig[api.status]?.color || 'default'} />
            </Stack>
            <Typography sx={{ mt: 2 }}>{api.description || 'No description provided.'}</Typography>
            <Divider sx={{ my: 2 }} />
          ...
          <AppCard title="Performance" subtitle="Recent product metrics" sx={{ mt: 3 }}>
            ...
            {tab === 1 && <List>{(api.plans || []).map((plan) => <ListItem key={plan.id} divider><ListItemText primary={plan.planName || 'Unnamed plan'} secondary={`${plan.billingCycle} • ${formatCurrency(plan.price)} • ${plan.requestLimit} req/mo`} /></ListItem>)}</List>}
            {tab === 2 && (
              <Box sx={{ mt: 2 }}>
                {api.subscribers > 0 ? (
                  <Typography color="text.secondary">This API currently has {api.subscribers} subscriber{api.subscribers === 1 ? '' : 's'}.</Typography>
                ) : (
                  <Typography color="text.secondary">This API does not have any active subscribers yet.</Typography>
                )}
              </Box>
            )}
          </AppCard>
        ...
        <Grid item xs={12} md={4}>
          <AppCard title="Technical details" subtitle="Integration context">
            <Stack spacing={1.2}>
              ...
              <Typography><strong>Auth:</strong> {api.authenticationType || 'Not returned'}</Typography>
