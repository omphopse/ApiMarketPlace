...
const AdminUserDetailPage = () => {
  ...
  return (
    <DashboardLayout role="ADMIN" title="User details" subtitle="Review account status and related data.">
      <PageHeader title={user.fullName || 'User details'} subtitle={user.email} action={<Stack direction="row" spacing={1}><Button variant="outlined" onClick={() => navigate('/admin/users')}>Back</Button><Button variant="contained" color="error" onClick={() => setSuspendOpen(true)}>Disable access</Button><Button variant="outlined" color="error" onClick={() => setDeleteOpen(true)}>Delete</Button></Stack>} />
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <AppCard title="Account actions" subtitle="Maintain access and trust">
            <Stack spacing={1}><Button fullWidth variant="contained" color="error" onClick={() => setSuspendOpen(true)}>Disable access</Button><Button fullWidth variant="outlined" onClick={handleReactivate}>Enable access</Button><Button fullWidth variant="outlined" color="error" onClick={() => setDeleteOpen(true)}>Delete user</Button></Stack>
          </AppCard>
        </Grid>
      </Grid>
